...
function IRRCalc() {
  ...
  const addRow = () => setFlows(f => [...f, { year: f.length, cf: 0 }]);
  const removeRow = (i) => setFlows(f => f.filter((_, idx) => idx !== i));
  const updateCF = (i, val) => setFlows(f => f.map((r, idx) => idx === i ? { ...r, cf: val } : r));

  const irrResult = useMemo(() => {
    const cfs = flows.map(f => f.cf);
  ...
  const npvData = useMemo(() => {
    const cfs = flows.map(f => f.cf);
    const points = [];
    for (let rate = 0; rate <= 0.30; rate += 0.005) {
      let npv = 0;
      for (let t = 0; t < cfs.length; t++) npv += cfs[t] / Math.pow(1 + rate, t);
      points.push({ rate: +(rate * 100).toFixed(1), npv: +npv.toFixed(0) });
    }
    return points;
  }, [flows]);
  ...
  const cumCF = useMemo(() => {
    let cum = 0;
    return flows.map(f => { cum += f.cf; return { year: f.year, cf: f.cf, cumCF: cum }; });
  }, [flows]);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        <Card>
          <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 12 }}>现金流输入</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {flows.map((f, i) => (
              <div key={i} style={{ display: "flex", gap: 8, alignItems: "center" }}>
                <span style={{ fontSize: 12, color: theme.textMuted, width: 50, fontFamily: "'JetBrains Mono'" }}>Y{f.year}</span>
                <input type="number" value={f.cf} onChange={e => updateCF(i, +e.target.value)}
                  style={{
                    flex: 1, background: theme.bg, border: `1px solid ${theme.border}`, borderRadius: 6,
                    padding: "6px 10px", color: f.cf >= 0 ? theme.green : theme.red, fontSize: 13,
                ...
                {flows.length > 2 && (
                  <button onClick={() => removeRow(i)} style={{
                    ...
                    color: theme.textDim, cursor: "pointer", padding: "4px 8px", fontSize: 12,
                  }}>×</button>
                )}
              </div>
            ))}
          </div>
          <Btn onClick={addRow} style={{ marginTop: 12, fontSize: 12 }}>+ 添加年份</Btn>
        </Card>

        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <Card><Stat label="IRR (内部收益率)" value={isFinite(irrResult) ? (irrResult * 100).toFixed(2) + "%" : "N/A"} color={irrResult > 0 ? theme.green : theme.red} /></Card>
          <Card>
            <Stat label="NPV @ 3.5%"
              value={(() => {
                const cfs = flows.map(f => f.cf);
                let npv = 0;
                for (let t = 0; t < cfs.length; t++) npv += cfs[t] / Math.pow(1.035, t);
                return npv.toFixed(0);
              })()}
              color={theme.accent} />
          </Card>
          <Card><Stat label="总现金流" value={flows.reduce((s, f) => s + f.cf, 0).toFixed(0)} color={theme.orange} /></Card>
        </div>
      </div>

      <Card>
        <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 12 }}>NPV 敏感性曲线 (折现率 vs NPV)</div>
        <ResponsiveContainer width="100%" height={280}>
          <ComposedChart data={npvData}>
            <CartesianGrid strokeDasharray="3 3" stroke={theme.border} />
            <XAxis dataKey="rate" stroke={theme.textDim} fontSize={11} label={{ value: "折现率 %", position: "bottom", fill: theme.textDim, fontSize: 11 }} />
            <YAxis stroke={theme.textDim} fontSize={11} />
            <Tooltip contentStyle={{ background: theme.surface, border: `1px solid ${theme.border}`, borderRadius: 8, fontSize: 12 }} />
            <Area type="monotone" dataKey="npv" stroke={theme.accent} fill={theme.accent} fillOpacity={0.1} name="NPV" />
            <ReferenceLine y={0} stroke={theme.red} strokeDasharray="3 3" />
          </ComposedChart>
        </ResponsiveContainer>
      </Card>

      <Card>
        <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 12 }}>现金流 & 累计现金流</div>
        <ResponsiveContainer width="100%" height={240}>
          <ComposedChart data={cumCF}>
            <CartesianGrid strokeDasharray="3 3" stroke={theme.border} />
            <XAxis dataKey="year" stroke={theme.textDim} fontSize={11} />
            <YAxis stroke={theme.textDim} fontSize={11} />
            <Tooltip contentStyle={{ background: theme.surface, border: `1px solid ${theme.border}`, borderRadius: 8, fontSize: 12 }} />
            <Legend />
            <Bar dataKey="cf" fill={theme.accent} fillOpacity={0.7} name="年现金流" />
            <Line type="monotone" dataKey="cumCF" stroke={theme.orange} strokeWidth={2} dot={false} name="累计现金流" />
            <ReferenceLine y={0} stroke={theme.textDim} strokeDasharray="3 3" />
          </ComposedChart>
        </ResponsiveContainer>
      </Card>
...
function LeeCarter() {
  ...
  const result = useMemo(() => {
    const t = TABLES[table];
    const data = gender === "male" ? t.male : t.female;
    const ages = Object.keys(data).map(Number).filter(a => a <= 100).sort((a, b) => a - b);
    const logMx = ages.map(a => Math.log((data[a] || 1) / 1000));

    // Simplified Lee-Carter: ax = mean(log mx), bx = normalized deviation, kt = drift * t
    const ax = logMx.map(v => v);
    const meanLog = logMx.reduce((s, v) => s + v, 0) / logMx.length;
    const bx = logMx.map(v => {
      const dev = v - meanLog;
      return dev;
    });
    const bxNorm = Math.sqrt(bx.reduce((s, v) => s + v * v, 0));
    const bxNormalized = bx.map(v => v / (bxNorm || 1));
    ...
    for (let yr = 0; yr <= forecastYears; yr++) {
      ...
      [20, 30, 40, 50, 60, 70, 80].forEach(age => {
        ...
        if (idx >= 0) {
          const forecastLog = ax[idx] + bxNormalized[idx] * kt;
          row[`age${age}`] = +(Math.exp(forecastLog) * 1000).toFixed(4);
        }
      });
      forecasts.push(row);
    }

    const currentVsForecast = ages.filter(a => a >= 20 && a <= 90).map(a => {
      const idx = ages.indexOf(a);
      const currentQ = (data[a] || 1);
      const kt = drift * forecastYears;
      const forecastLog = ax[idx] + bxNormalized[idx] * kt;
      const forecastQ = +(Math.exp(forecastLog) * 1000).toFixed(4);
      return { age: a, current: currentQ, forecast: forecastQ };
    });

    return { forecasts, currentVsForecast, ages };
  }, [table, gender, forecastYears, drift]);

  const ageColors = {
    age20: theme.green, age30: theme.accent, age40: theme.purple,
    age50: theme.orange, age60: theme.pink, age70: theme.red, age80: theme.yellow,
  };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
        ...
        <Select label="性别" value={gender} onChange={setGender}
          options={[{ value: "male", label: "男" }, { value: "female", label: "女" }]} />
        <Input label="预测年数" value={forecastYears} onChange={setForecastYears} min={5} max={50} style={{ width: 100 }} />
        <Input label="kt漂移率" value={drift} onChange={setDrift} step={0.005} style={{ width: 100 }} />
      </div>

      <Card style={{ padding: 16, background: theme.surface, border: `1px solid ${theme.border}`, borderRadius: 8 }}>
        <div style={{ fontSize: 12, color: theme.textMuted, lineHeight: 1.8 }}>
          <strong style={{ color: theme.text }}>Lee-Carter 模型：</strong> ln(m(x,t)) = a(x) + b(x)·k(t)。
          a(x) 为各年龄平均对数死亡率，b(x) 为年龄敏感度，k(t) 为时间趋势。
          漂移率 drift 控制 k(t) 的年变化量，负值表示死亡率逐年改善。
        </div>
      </Card>

      <Card>
        <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 12 }}>死亡率预测趋势 (‰)</div>
        <ResponsiveContainer width="100%" height={340}>
          <LineChart data={result.forecasts}>
            <CartesianGrid strokeDasharray="3 3" stroke={theme.border} />
            <XAxis dataKey="year" stroke={theme.textDim} fontSize={11} label={{ value: "预测年", position: "bottom", fill: theme.textDim, fontSize: 11 }} />
            <YAxis stroke={theme.textDim} fontSize={11} />
            <Tooltip contentStyle={{ background: theme.surface, border: `1px solid ${theme.border}`, borderRadius: 8, fontSize: 12 }} />
            <Legend />
            {Object.entries(ageColors).map(([key, color]) => (
              <Line key={key} type="monotone" dataKey={key} stroke={color} strokeWidth={2} dot={false}
                name={`${key.replace("age", "")}岁`} />
            ))}
          </LineChart>
        </ResponsiveContainer>
      </Card>
      ...
      <Card>
        <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 12 }}>当前 vs {forecastYears}年后预测 (‰)</div>
        <ResponsiveContainer width="100%" height={300}>
          <LineChart data={result.currentVsForecast}>
            <CartesianGrid strokeDasharray="3 3" stroke={theme.border} />
            <XAxis dataKey="age" stroke={theme.textDim} fontSize={11} />
            <YAxis stroke={theme.textDim} fontSize={11} scale="log" domain={["auto", "auto"]}
              tickFormatter={v => v >= 1 ? v.toFixed(0) : v.toFixed(2)} />
            <Tooltip contentStyle={{ background: theme.surface, border: `1px solid ${theme.border}`, borderRadius: 8, fontSize: 12 }} />
            <Legend />
            <Line type="monotone" dataKey="current" stroke={theme.accent} strokeWidth={2} dot={false} name="当前 qx" />
            <Line type="monotone" dataKey="forecast" stroke={theme.orange} strokeWidth={2} dot={false} strokeDasharray="6 3" name={`${forecastYears}年后 qx`} />
          </LineChart>
        </ResponsiveContainer>
      </Card>
    </div>
...
// ==========================================
function AgriInsurance() {
  const [crop, setCrop] = useState("wheat");
  const [area, setArea] = useState(100);
  const [unitYield, setUnitYield] = useState(500);
  const [price, setPrice] = useState(2.8);
  const [coverage, setCoverage] = useState(0.8);
  const [lossRate, setLossRate] = useState(0.08);
  const [loadingFactor, setLoadingFactor] = useState(0.3);
  const [subsidy, setSubsidy] = useState(0.8);
  const [deductible, setDeductible] = useState(0.1);
  const [simYears, setSimYears] = useState(20);

  const crops = [
    { value: "wheat", label: "小麦", baseRate: 0.06, volatility: 0.25 },
    { value: "rice", label: "水稻", baseRate: 0.05, volatility: 0.20 },
    { value: "corn", label: "玉米", baseRate: 0.07, volatility: 0.28 },
    { value: "soybean", label: "大豆", baseRate: 0.08, volatility: 0.30 },
    { value: "cotton", label: "棉花", baseRate: 0.09, volatility: 0.35 },
    { value: "rapeseed", label: "油菜", baseRate: 0.07, volatility: 0.27 },
  ];

  const result = useMemo(() => {
    const cropInfo = crops.find(c => c.value === crop) || crops[0];
    const totalValue = area * unitYield * price;
    const insuredValue = totalValue * coverage;
    const purePremiumRate = lossRate;
    const grossPremiumRate = purePremiumRate * (1 + loadingFactor);
    const purePremium = insuredValue * purePremiumRate;
    const grossPremium = insuredValue * grossPremiumRate;
    const farmerPremium = grossPremium * (1 - subsidy);
    const govSubsidy = grossPremium * subsidy;
    ...
    let totalLoss = 0, totalPrem = 0;
    for (let yr = 1; yr <= simYears; yr++) {
      const yieldShock = normalRandom() * cropInfo.volatility;
      const actualYieldRatio = Math.max(0, 1 + yieldShock);
      const actualYield = unitYield * actualYieldRatio;
      const lossRatio = Math.max(0, 1 - actualYieldRatio);
      const claimable = Math.max(0, lossRatio - deductible);
      const claim = insuredValue * claimable;
      totalLoss += claim;
      totalPrem += grossPremium;

      scenarios.push({
        year: yr,
        actualYield: +actualYield.toFixed(0),
        yieldRatio: +(actualYieldRatio * 100).toFixed(1),
        lossRatio: +(lossRatio * 100).toFixed(1),
        claim: +claim.toFixed(0),
        premium: +grossPremium.toFixed(0),
        profit: +(grossPremium - claim).toFixed(0),
      });
    }

    const simLossRatio = totalLoss / totalPrem;

    return {
      totalValue, insuredValue, purePremiumRate, grossPremiumRate,
      purePremium, grossPremium, farmerPremium, govSubsidy,
      scenarios, simLossRatio, cropInfo,
    };
  }, [crop, area, unitYield, price, coverage, lossRate, loadingFactor, subsidy, deductible, simYears]);

  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
        <Select label="作物品种" value={crop} onChange={setCrop}
          options={crops.map(c => ({ value: c.value, label: c.label }))} />
        <Input label="种植面积(亩)" value={area} onChange={setArea} min={1} style={{ width: 100 }} />
        <Input label="单产(kg/亩)" value={unitYield} onChange={setUnitYield} style={{ width: 110 }} />
        <Input label="价格(元/kg)" value={price} onChange={setPrice} step={0.1} style={{ width: 100 }} />
        <Input label="保障水平" value={coverage} onChange={setCoverage} step={0.05} style={{ width: 90 }} />
        <Input label="纯费率" value={lossRate} onChange={setLossRate} step={0.01} style={{ width: 80 }} />
        <Input label="附加费率" value={loadingFactor} onChange={setLoadingFactor} step={0.05} style={{ width: 90 }} />
        <Input label="政府补贴" value={subsidy} onChange={setSubsidy} step={0.05} style={{ width: 90 }} />
        <Input label="免赔率" value={deductible} onChange={setDeductible} step={0.05} style={{ width: 80 }} />
        <Input label="模拟年数" value={simYears} onChange={setSimYears} min={5} max={100} style={{ width: 90 }} />
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16 }}>
        <Card><Stat label="总产值" value={`¥${(result.totalValue / 10000).toFixed(1)}万`} color={theme.text} /></Card>
        <Card><Stat label="保险金额" value={`¥${(result.insuredValue / 10000).toFixed(1)}万`} color={theme.accent} /></Card>
        <Card><Stat label="总保费" value={`¥${result.grossPremium.toFixed(0)}`} color={theme.orange} /></Card>
        <Card><Stat label="农户自缴" value={`¥${result.farmerPremium.toFixed(0)}`} color={theme.green} sub={`政府补贴 ¥${result.govSubsidy.toFixed(0)}`} /></Card>
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
        <Card><Stat label="纯费率" value={(result.purePremiumRate * 100).toFixed(1) + "%"} color={theme.purple} /></Card>
        <Card><Stat label="毛费率" value={(result.grossPremiumRate * 100).toFixed(1) + "%"} color={theme.pink} /></Card>
        <Card><Stat label="模拟赔付率" value={(result.simLossRatio * 100).toFixed(1) + "%"} color={result.simLossRatio > 1 ? theme.red : theme.green} sub={`${simYears}年蒙特卡洛`} /></Card>
      </div>

      <Card>
        <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 12 }}>模拟赔付 vs 保费</div>
        <ResponsiveContainer width="100%" height={300}>
          <ComposedChart data={result.scenarios}>
            <CartesianGrid strokeDasharray="3 3" stroke={theme.border} />
            <XAxis dataKey="year" stroke={theme.textDim} fontSize={11} />
            <YAxis stroke={theme.textDim} fontSize={11} />
            <Tooltip contentStyle={{ background: theme.surface, border: `1px solid ${theme.border}`, borderRadius: 8, fontSize: 12 }} />
            <Legend />
            <Bar dataKey="claim" fill={theme.red} fillOpacity={0.6} name="赔付" />
            <Line type="monotone" dataKey="premium" stroke={theme.green} strokeWidth={2} dot={false} name="保费" />
            <ReferenceLine y={0} stroke={theme.textDim} />
          </ComposedChart>
        </ResponsiveContainer>
      </Card>

      <Card>
        <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 12 }}>产量波动模拟 (%)</div>
        <ResponsiveContainer width="100%" height={240}>
          <ComposedChart data={result.scenarios}>
            <CartesianGrid strokeDasharray="3 3" stroke={theme.border} />
            <XAxis dataKey="year" stroke={theme.textDim} fontSize={11} />
            ...
            <Tooltip contentStyle={{ background: theme.surface, border: `1px solid ${theme.border}`, borderRadius: 8, fontSize: 12 }} />
            <Legend />
            <Bar dataKey="yieldRatio" fill={theme.accent} fillOpacity={0.6} name="实际产量 %" />
            <ReferenceLine y={100} stroke={theme.orange} strokeDasharray="3 3" label={{ value: "正常", fill: theme.orange, fontSize: 11 }} />
            <ReferenceLine y={(1 - deductible) * 100} stroke={theme.red} strokeDasharray="3 3" label={{ value: "免赔线", fill: theme.red, fontSize: 11 }} />
          </ComposedChart>
        </ResponsiveContainer>
      </Card>

      <Card style={{ overflowX: "auto" }}>
        <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 12 }}>逐年模拟明细</div>
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, fontFamily: "'JetBrains Mono', monospace" }}>
          <thead>
            <tr style={{ borderBottom: `1px solid ${theme.border}` }}>
              {["年", "实际单产", "产量%", "损失%", "赔付", "保费", "损益"].map(h => (
                <th key={h} style={{ padding: "8px 6px", textAlign: "right", color: theme.textMuted, fontWeight: 500 }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {result.scenarios.map(r => (
              <tr key={r.year} style={{ borderBottom: `1px solid ${theme.border}` }}>
                <td style={{ padding: "6px", textAlign: "right", color: theme.text }}>{r.year}</td>
                <td style={{ padding: "6px", textAlign: "right", color: theme.textMuted }}>{r.actualYield}</td>
                <td style={{ padding: "6px", textAlign: "right", color: r.yieldRatio < 100 ? theme.red : theme.green }}>{r.yieldRatio}%</td>
                <td style={{ padding: "6px", textAlign: "right", color: r.lossRatio > 0 ? theme.red : theme.textMuted }}>{r.lossRatio}%</td>
                <td style={{ padding: "6px", textAlign: "right", color: theme.red }}>{r.claim.toLocaleString()}</td>
                <td style={{ padding: "6px", textAlign: "right", color: theme.green }}>{r.premium.toLocaleString()}</td>
                <td style={{ padding: "6px", textAlign: "right", color: r.profit >= 0 ? theme.green : theme.red }}>{r.profit.toLocaleString()}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
export default function App() {
  ...
  const tabs = [
    ...
    { key: "irr", label: "IRR" },
    { key: "leecarter", label: "Lee-Carter" },
    { key: "agri", label: "农险定价" },
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: "24px 32px", maxWidth: 1200, margin: "0 auto" }}>
        ...
        {tab === "irr" && <IRRCalc />}
        {tab === "leecarter" && <LeeCarter />}
        {tab === "agri" && <AgriInsurance />}
